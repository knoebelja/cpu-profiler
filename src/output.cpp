...
void print_event(const stack_event &event, SymbolResolver &resolver, int stack_traces_fd) {
    ...
    // These show as "path+offset" for now since we haven't parsed ELF symbol tables yet.
    auto user_frames = resolver.resolve_user_stack(stack_traces_fd, event.user_stack_id, event.pid);
